	...
	if [ "$STATUS" != "ACTIVE" ]; then
		...
		if [ -e "./services/$SERVICE_NAME/rule.json" ]; then
			...
			RESULT=`aws ecs create-service \
				...
				## The service is created, register as scalable target for service autoscaling.
		        printf "${PRIMARY}* Registering Service \`${SERVICE_NAME}\` as scalable target \n";
				...
				# The scalable target is registered, just update the service autoscaling.
				printf "${PRIMARY}* Updating service \`${SERVICE_NAME}\` with task autoscaling with target tracking policy \n";
				RESULT=`aws application-autoscaling put-scaling-policy \
				--region $REGION \
				--service-namespace ecs \
				--resource-id service/$CLUSTER_NAME/$SERVICE_NAME \
				--policy-name TargetTrackingPolicy-$SERVICE_NAME \
				--scalable-dimension ecs:service:DesiredCount \
				--policy-type TargetTrackingScaling \
				--target-tracking-scaling-policy-configuration "{ \"TargetValue\": 70.0, \"PredefinedMetricSpecification\": { \"PredefinedMetricType\": \"ECSServiceAverageCPUUtilization\" }, \"ScaleOutCooldown\": 30, \"ScaleInCooldown\": 300}"`
		else
