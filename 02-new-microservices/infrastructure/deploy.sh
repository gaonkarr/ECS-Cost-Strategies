	...
	if [ "$STATUS" != "ACTIVE" ]; then
		...
		if [ -e "./services/$SERVICE_NAME/rule.json" ]; then
			...
			RESULT=`aws ecs create-service \
				...
				--target-tracking-scaling-policy-configuration "{ \"TargetValue\": 50.0, \"PredefinedMetricSpecification\": { \"PredefinedMetricType\": \"ECSServiceAverageCPUUtilization\" }, \"ScaleOutCooldown\": 10, \"ScaleInCooldown\": 30}"`
		else
